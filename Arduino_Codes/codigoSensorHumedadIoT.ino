...
void loop() {
  ...
  float humedadPromedio = 40;
  // We print each value along with its unit of measure
  // Se hace la impresión de cada valor junto con su unidad de medida
  Serial.print("Humedad = ");
  Serial.print(h);
  Serial.print("% ");
  if (h > humedadPromedio){
    ...
    Serial.println("Humedad normal"); // Se apaga el actuador cuando las lecturas son normales / We turn off the actuator when the readings are normal
  }
  ...
  if ((WiFi.status() == WL_CONNECTED)) {
    ...
    http.begin(client, "http://000.000.000.000:3000/api/insertLogHumedad");  // Replace with your IP and port
    ...
    if (h > humedadPromedio){
      ...
      doc["id_sensor"] = 3; // Ajustar segun el sensor de donde se este obteniendo la lectura / Adjust according to the sensor from which the reading is being obtained
